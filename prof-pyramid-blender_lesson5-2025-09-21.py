"""
Defines the initial Rithmomachia board setup and an educational animation.
This version refactors the animation sequence to separate text/highlighting
scenes from piece movement scenes for clearer, more reliable timing.

Adds a shininess parameter for the pieces.

This is a lesson on the pyramids.

write a function to 
(a) populate the board from the initial position (as a global variable),
(b) clear the board via a for loop of animated "captures" (moves by (50, -50),
as usual), and
(c) create another animation of pieces populating the boards 
    for a new position (as a local variable). This is obtained by
    "un-moving" each piece on the new board (moves by (-50, 50), as usual),
    followed by a suitable shift.
NOTE: The new position must be a board for which we know, for each piece on
the new board, the move needed to shift to that location from the initial board.
In particular, each piece must be legally placed. 

explain 
(a) briefly the subpieces of the pyramid,
(b) how the pyramid moves and captures, 
(c) examples of all these rules:
    * (if white's move) (S25 * 4) x t100, followed by
                        S25 x (s25-as-subpiece-of-p190) (capture by number), so p190 -> p175, OR
      (if black's move) (s25-as-subpiece-of-p190) x S25 (capture by number), 
    * start with C8 move by (1,0), followed by 
      (if white's move) (S36-as-subpiece-of-P91 / 4)xc9, followed by
                        (S25-as-subpiece-of-P91 / 5)xc5, followed by
                        (P91 / 13)xc7, OR
      (if black's move) (c9 * 4) x (S36-as-subpiece-of-P91), followed by
                        (c5 * 5) x (S25-as-subpiece-of-P91), followed by
                        (c7 * 13) x P91.
   *  (put back P91) start with T42 move by (0,2), followed by 
      (if white's move) (S16-as-subpiece-of-P91 * 1)xt16, OR
      (if black's move) (t16 * 1) x (S16-as-subpiece-of-P91), so P91 -> P75, followed by
                        (c25 * 3) x P75,

This is lesson 5, based on the blender python module rithmomachia_pieces_blender+pyramid1b

last modified by wdj on 2025-09-21
"""
import bpy
import copy
import bmesh
...
CUSTOM_GAME_BOARD2 = [
    ...
    [''      , 'T020_1', 'c005_1', 'C006_1', ''      , '', ''      , '', ''      , ''      , ''      , 'c007_1', 'C002_1', 'c049_1', 't056_1', ''      ],
    [''      , 'T025_1', 'C004_2', ''      , ''      , '', ''      , '', ''      , ''      , ''      , 'C016_1', 'c009_1', 'c081_1', 't064_1', ''      ], 
    ['S081_1', 'S045_1', ''      , ''      , ''      , '', ''      , '', 'T006_1', ''      , ''      , ''      , ''      , 't090_1', 's120_1', 's225_1'],
...
def create_educational_animation_sequence():
    """
    Defines the educational animation sequence using a list of dictionaries.
    
    The structure is separated into "text_and_highlight" scenes and "move_piece"
    scenes to prevent conflicts and provide clearer timing control.
    
    Each dictionary must have a 'scene_type' key.
    
    Returns:
        list: A list of dictionaries, where each dictionary defines an
              animation scene.


    ####### description of CUSTOM_GAME_BOARD combinations
    #    * (if white's move) (S25 * 4) x t100, followed by
    #                    S25 x (s25-as-subpiece-of-p190) (capture by number), so p190 -> p175, OR
    #      (if black's move) (s25-as-subpiece-of-p190) x S25 (capture by number), followed by
    #    * start with C8 move by (1,0), followed by 
    #     (if white's move) (S36-as-subpiece-of-P91 / 4)xc9, followed by
    #                    (S25-as-subpiece-of-P91 / 5)xc5, followed by
    #                    (P91 / 13)xc7, OR
    ...
    animation_sequence = [
        #Scene 0a: An introduction to Prof Pyramid
         ...
         #   'pause': 10
         #},
        # Scene 0b: An introduction to pyramid captures
        {
            'scene_type': 'text_and_highlight',
            'move_text': "  In this tutorial we learn how the pyramid pieces move and capture, with examples from the study below.   ",
            'highlighted_pieces': [],## use highlighted_empty_squares instead
            'highlighted_empty_squares': [(1,1),(1,5),(1,6),(1,7),(1,15), (2,1),(3,1),(5,1), (7,6),(7,11),(7,14)],
            'pause': 10,
        },
        ...
        # Scene 1a: Explain the white pyramid
        {
            'scene_type': 'text_and_highlight',
            'move_text': "The white pyramid has a total value of 91, composed of 6 sub-pieces, squares of values 1, 4, 9, 16, 25, and 36, respectively.",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1)],
            'pause': 10,
        },
        # Scene 1b1: Explain the white pyramid
        {
            'scene_type': 'text_and_highlight',
            'move_text': "It moves and captures like a square piece of value 91, but it can do much more.",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1)],
            'pause': 10,
        },
        # Scene 1b2: Explain the white pyramid
        {
            'scene_type': 'text_and_highlight',
            'move_text': "The pyramid piece moves in unison with all its subpieces. However, the good news is each sub-piece can capture independently of each other. The bad news is that they can also themselves be captured independently of each other!",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1)],
            'pause': 10
        },
        ...
        {
            'scene_type': 'move_piece',
            'piece_name': "SpherePyramid65321GraphicS",
            'steps': (-410, 0),
            'pause': 10,
        },
        # Scene 1c: Explain the white pyramid
        {
            'scene_type': 'text_and_highlight',
            'move_text': "Not only that, but if a subpiece of the pyramid captured then the pyramid's total value must be adjusted accordingly. For example, if P91's subpiece of value 16 is captured, P91 becomes P75 (since 91 - 16 = 75).", ## "minus"
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1)],
            'pause': 10,
        },
        ...
        {
            'scene_type': 'move_piece',
            'piece_name': "SpherePyramid87654GraphicS",
            'steps': (-420, 0),
            'pause': 10,
        },
        # Scene 2a: Explain the black pyramid
        {
            'scene_type': 'text_and_highlight',
            'move_text': "The black pyramid has a total value of 190, composed of 5 sub-pieces (namely, squares of values 16, 25, 36, 49, and 64, respectively).",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(7,14)],
            'pause': 10,
        },
        # Scene 2b: Explain the black pyramid
        {
            'scene_type': 'text_and_highlight',
            'move_text': "Likewise, for P190: it moves and captures like a square piece of value 190 while each sub-piece can capture independently, as a square piece of value 16, 25, 36, 49, or 64.",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(7,14)],
            'pause': 10
        },
        ...
        {
            'scene_type': 'move_piece',
            'piece_name': "SpherePyramid8654GraphicS",
            'steps': (-430, 0),
            'pause': 10,
        },
        # Scene 2b: Explain the black pyramid
        {
            'scene_type': 'text_and_highlight',
            'move_text': "Here too, the subpieces can be captured and, once captured, the pyramid's value must be adjusted accordingly. For example, if p190's subpiece of value 49 is captured, p190 becomes p141 (since 190 - 49 = 141). ", ## "minus"
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(7,14)],
            'pause': 10,
        },
        # Scene 2b: Explain the pyramid
        {
            'scene_type': 'text_and_highlight',
            'move_text': "That these subpieces are so numerous, each one having 5 or 6 of them, makes the pyramid the most powerful piece in its color.",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(7,14),(1,1)],
            'pause': 10,
        },
        # Scene 1a2
        {
        ...
        # Scene 3: Explain the white and black pyramids
        {
            'scene_type': 'text_and_highlight',
            'move_text': "Since each pyramid moves three spaces orthogonally, the opposing pyramids can't face-off one move away from each other. This is fortunate because if they could Capture By Number would allow one of them to capture three of the five sub-pieces (namely, the squares whose values 16, 25, 36 they have in common)!",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1), (7,14)],
            'pause': 10,
        },
        # Scene 4a: Explain the black pyramid captures
        #    * (if white's move) (S25 * 4) x t100, followed by
        #                    S25 x (s25-as-subpiece-of-p190) (capture by number), so p190 -> p175, OR
        #      (if black's move) (s25-as-subpiece-of-p190) x S25 (capture by number), 
        {
            'scene_type': 'text_and_highlight',
            'move_text': "The black pyramid is involved with a few captures. Let's look at them.",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(7,14),(7,6),(7,11)],
            'pause': 10,
        },
        # Scene 4b:
        {
            'scene_type': 'text_and_highlight',
            'move_text': "If it's Black's move, then s25-as-a-subpiece-of-p190 can Capture By Number S25, since it is One move away.",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(7,14),(7,11)],
            'pause': 10,
        },
        # Scene 4c:
        {
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "We'll put that back, as we'll need it for later.",
            'highlighted_pieces': [], 
            'highlighted_empty_squares': [ (7,14),(7,11) ],
            'pause': 10
        },
        # Scene 4e: An un-capture for that piece
        {
            'scene_type': 'move_piece',
            'piece_name': 'S025_1',
            'steps': (50, -50),
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "If it's White's move, then S25 can Capture By Number s25-as-a-subpiece-of-p190, since it too is One move away. This changes p190 to p165.",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(7,14),(7,11)],
            'pause': 10
        },
        # Scene 4g: ## remove p190 and replace it by p165 - but how? ii = 7, jj = 14, unshift from (ii - 50 - 25, jj + 50 + 25) to (7,14)
        {
            'scene_type': 'move_piece',
            'piece_name': 'p190_1',
            'steps': (50, -50),
        ...
        # Scene 4h: ## remove p190 and replace it by p165 - but how? ii = 7, jj = 14, unshift from (ii - 50 - 25, jj + 50 + 25) to (7,14)
        {
            'scene_type': 'move_piece',
            'piece_name': 'p165_1',
            'steps': (75, -75),
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "The white pyramid has its share of captures, as well. Let's look at them.",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1),(1,5),(1,6),(1,7),(1,15)],
            'pause': 10,
        },
        # Scene 5b:
        {
            'scene_type': 'text_and_highlight',
            'move_text': "Look what happens if White moves C8, then there are a lot of potential captures to search through.",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1),(1,5),(1,6),(1,7),(1,15)],
            'pause': 10,
        },
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "Each turn is (a) a capture (optional), (b) a piece move, (c) a capture (optional). In this position, if it's still White's move, then there are several captures.",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1),(1,6),(1,7),(1,15)],
            'pause': 10
        },
        #    Scene 5d: 
        {
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "White's S36-as-a-subpiece-of-P91 can Capture By Divisor (d = 4) the circle c9, since it is 4 spaces away.",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1),(1,6),(1,7),(1,15)],
            'pause': 10,
        },
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "Now that black's circle is gone, a new black piece is revealed to be in a line with P91. In fact, White's S25-as-a-subpiece-of-P91 can Capture By Divisor (d = 5) the circle c5, since it is 5 spaces away (and 25/5 = 5).",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1),(1,7),(1,15)],
            'pause': 10
        },
        #    Scene 5f: 
        {
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "Deja vu. With black's circle c5 gone, another new black piece is revealed to be in a line with P91. In fact, P91 can Capture By Divisor (d = 13) the circle c7, since it is 13 spaces away (and 91/13 = 7).",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1),(1,15)],
            'pause': 10
        },
        #    Scene 5f: 
        {
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "That is three piece captures in one turn!",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [],
            'pause': 10
        },
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "Let's return all those pieces and look at a different White move.",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1),(1,5),(1,6),(1,7),(1,15)],
            'pause': 10
        },
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "With C8 out of the way, do you see the captures Black could make if it was Black's turn? (Pause the video if you need to.)",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1),(1,5),(1,6),(1,7),(1,15)],
            'pause': 30
        },
        ...
        # Scene 6:
        # return all the pieces and suppose instead of C8, White
        # start with T42 move by (0,2), followed by 
        # (if white's move) (S16-as-subpiece-of-P91 * 1)xt16, OR
        # (if black's move) (t16 * 1) x (S16-as-subpiece-of-P91), so P91 -> P75, followed by
        #                (c25 * 3) x P75,
        # Scene 6a:
        {
            'scene_type': 'text_and_highlight',
            'move_text': "What if White, instead of the circle C8, moves the triangle T49?",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [],
            'pause': 10
        },
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "White's S16-as-a-subpiece-of-P91 can Capture By Multiple (d = 1) the triangle t16, since it is 1 space away.",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1),(3,1)],
            'pause': 10
        },
        #    Scene 6d: 
        {
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "Let's put that back and ask: what captures there are if it's Black's turn?",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1),(3,1),(5,1)],
            'pause': 10
        },
        #    Scene 6f: 
        {
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "Black's triangle t16 can Capture By Multiple (d = 1) S16-as-a-subpiece-of-P91, since it is 1 space away. This transforms P91 to P75.",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1),(3,1)],
            'pause': 10
        },
        # Scene 6h: ## remove P91 and replace it by P75 - but how? ii = 1, jj = 1, unshift from (ii - 50 - 16, jj + 50 + 16) to (1,1)
        {
            'scene_type': 'move_piece',
            'piece_name': 'P091_1',
            'steps': (50, -50),
        ...
        # Scene 6i: ## remove p190 and replace it by p165 - but how? ii = 7, jj = 14, unshift from (ii - 50 - 25, jj + 50 + 25) to (7,14)
        {
            'scene_type': 'move_piece',
            'piece_name': 'P075_1',
            'steps': (66, -66),
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "That was his first (pre-move) capture. Black now moves triangle t16 out of the way, to reveal c25.",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1),(3,1)],
            'pause': 10
        },
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "For the post-move capture, Black's circle c25 can Capture By Multiple (d = 3) P75, since it is 3 spaces away.",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [(1,1),(5,1)],
            'pause': 10
        },
        # Scene 6m: ## remove p190 and replace it by p165 - but how? ii = 7, jj = 14, unshift from (ii - 50 - 25, jj + 50 + 25) to (7,14)
        {
            'scene_type': 'move_piece',
            'piece_name': 'P075_1',
            'steps': (-66, 66),
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "That is two piece captures in one turn, including the pyramid!",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [],
            'pause': 10
        },
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "While this study position will likely never arise in a real game, it shows the variety of piece captures the pyramid can be involved with.",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [],
            'pause': 10
        },      
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': "Including the previous lessons, we have now covered all the methods to capture any rithmomachia piece. Amazing how all the logic of the rules fits together, isn't it?",
            'highlighted_pieces': [],## use highlighted_empty_squares instead
            'highlighted_empty_squares': [],
            'pause': 10,
        },
        ...
        {
            'scene_type': 'text_and_highlight',
            'move_text': " Thank you for watching this video about piece captures involving the Pyramid piece. This is Professor Pyramid signing off. I hope to see you in another rithmomachia lesson soon! ",
            'highlighted_pieces': [],
            'highlighted_empty_squares': [ ],
            'pause': 10
        },
...
def main_integrated_animation():
    ...
    # This example creates S36xc9 (d=4)
    stack_location = Vector((6+110, -7, 0.0)) # Position it to the left-center of the board
    text_obj = create_3d_text(
    text="36 divided by 4 equals 9, so S36 can take c9.",
    location=stack_location,
    font_size=0.9,
    color_name="off_white+lightblue",
    color_rgba=(0.6, 0.7, 0.7, 1.0),
    ...
    #####################
    # This example creates S25 x c5
    stack_location = Vector((6 + 120, -7, 0)) # Position it to the left-center of the board
    text_obj = create_3d_text(
    text="25 divided by 5 equals 5, so S25 can take c5.",
    location=stack_location,
    font_size=1.0,
    ...
    #####################
    # This example creates 91/13=7 so P91xc7
    stack_location = Vector((6 + 130, -7, 0)) # Position it to the left-center of the board
    text_obj = create_3d_text(
    text="91 divided by 13 equals 7, so P91 can take c7",
    location=stack_location,
    font_size=0.9,
    color_name="off_white",
    color_rgba=(0.6, 0.7, 0.7, 1.0),
    ...
    text_obj = create_3d_text(
    text="25 times 3 equals 75, so s25 can take P75.",
    location=stack_location,
    font_size=0.9,
    color_name="off_white",
    color_rgba=(0.6, 0.7, 0.7, 1.0),
    ...
    text_obj = create_3d_text(
    text="16/2 = 8   and    t16 is distance d=2 from C8.",
    location=stack_location,
    font_size=0.9,
    color_name="off_white",
    color_rgba=(0.6, 0.7, 0.7, 1.0),
    ...
    #    #####################
    # Create 12/3=4
    stack_location = Vector((1 + 170, -7, 0)) # Position it to the left-center of the board
    text_obj = create_3d_text(
    text="12/3 = 4   and    t12 is distance d=3 from C4.",
    location=stack_location,
    font_size=0.9,
    color_name="off_white",
    color_rgba=(0.6, 0.7, 0.7, 1.0),
    ...
    text_obj = create_3d_text(
    text="12 times 6 = 72   and   t12 is distance d=6 from T72.",
    location=stack_location,
    font_size=0.9,
    color_name="off_white",
    color_rgba=(0.6, 0.7, 0.7, 1.0),
    ...
    pyramid_graphic = create_stacked_slabs_graphic(
        ...
        diameter=0.6,              # Diameter of each sphere
        sep_dist=0.05,              # Small gap between spheres
        name="SpherePyramid6-1GraphicS", # Unique name for the object
        stack_color = (0.5, 0.5, 1.0, 1.0) #  blue-ish
    )
    # Add it to the dictionary so the animation system can find it
    pieces_on_board["SpherePyramid6-1GraphicS"] = pyramid_graphic
    #      #####################
    stack_location = Vector((8 + 410, -4, 2)) # Position it to the left-center of the board
    pyramid_graphic = create_stacked_slabs_graphic(
        location=stack_location,
        slab_sizes=[6, 5, 3, 2, 1], # Creates a pyramid shape
        obj_type="cube",         # Use spheres as building blocks
        diameter=0.6,              # Diameter of each sphere
        sep_dist=0.05,              # Small gap between spheres
        name="SpherePyramid65321GraphicS", # Unique name for the object
        stack_color = (0.5, 0.5, 1.0, 1.0) #  blue-ish
    )
    # Add it to the dictionary so the animation system can find it
    pieces_on_board["SpherePyramid65321GraphicS"] = pyramid_graphic
    #      #####################
    stack_location = Vector((8 + 420, -4, 2)) # Position it to the left-center of the board
    pyramid_graphic = create_stacked_slabs_graphic(
        location=stack_location,
        ...
        diameter=0.6,              # Diameter of each sphere
        sep_dist=0.05,              # Small gap between spheres
        name="SpherePyramid87654GraphicS", # Unique name for the object
        stack_color = (1.0, 0.5, 0.5, 1.0) #  red-ish
    )
    # Add it to the dictionary so the animation system can find it
    pieces_on_board["SpherePyramid87654GraphicS"] = pyramid_graphic
    #      #####################
    stack_location = Vector((8 + 430, -4, 2)) # Position it to the left-center of the board
    pyramid_graphic = create_stacked_slabs_graphic(
        location=stack_location,
        slab_sizes=[8, 6, 5, 4], # Creates a pyramid shape
        obj_type="cube",         # Use spheres as building blocks
        diameter=0.6,              # Diameter of each sphere
        sep_dist=0.05,              # Small gap between spheres
        name="SpherePyramid8654GraphicS", # Unique name for the object
        stack_color = (1.0, 0.5, 0.5, 1.0) #  red-ish
    )
    # Add it to the dictionary so the animation system can find it
    pieces_on_board["SpherePyramid8654GraphicS"] = pyramid_graphic
